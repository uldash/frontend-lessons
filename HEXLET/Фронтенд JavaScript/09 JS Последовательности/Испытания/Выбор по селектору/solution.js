...
Пример работы функции, где для наглядности показано какой она
будет возвращать результат если выводить его на экран (htmlToString):
...
query ищется с любого уровня дерева, а не только с верхнего. Например, если наш query это p,
то будут найдены все p на всех уровнях.
Производится поиск только подряд идущих тегов, например, если запрос l('ul', 'li'),
то будут найдены только те li, которые идут сразу за ul.
...
import { node, append, make, is, toString as htmlToString, hasChildren, children, reduce, filter, map } from 'hexlet-html-tags'; // eslint-disable-line

const getChildrenFromElements = (elements) => {
  const mapped = map((element) => (hasChildren(element) ? children(element) : l()), elements);
  const result = reduce(concat, l(), mapped);
  return result;
};
