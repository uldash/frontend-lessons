...
Допишите необходимые части функций bigLettersCount и compare для того, чтобы заработали функции 
lessThan, greaterThan и isEqual.
...
assert.equal(greaterThan('AD', 'ad sd'), true); // true, сравнение на > (больше)
assert.equal(greaterThan('AD', '   Ad sd'), false); // false, сравнение на > (больше)
assert.equal(lessThan('ghe df', 'dfwe r D'), true); // true, сравнение на < (меньше)
assert.equal(isEqual('liSp', 'lisP'), true); // true
